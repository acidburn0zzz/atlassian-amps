...
def obrFile = new File(basedir, 'target/amps-maven-plugin-genenerate-obr-artifact-test-testing.obr')
assert obrFile.exists(), "The $obrFile file should exist"

final String unzipLocation = "$basedir/target/obrunzip"
final String obrLocation = "$basedir/target/amps-maven-plugin-genenerate-obr-artifact-test-testing.obr"

ZipFile obrCompressed = new ZipFile(obrLocation)
obrCompressed.extractAll(unzipLocation)


assert new File(unzipLocation, 'amps-maven-plugin-genenerate-obr-artifact-test-testing.jar').exists()
